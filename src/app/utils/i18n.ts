...
export class Localization {
  ...
  getLocalizedMsg = (option: ILocalizeMessage, lang?: any): string => {
    ...
    if (!msg || msg === option.key) {
      msg = `<err: localized msg not found in json files><flag: ${option.key}><lang: ${this.i18n.getLocale()}>`
    }
...
export const localization = new Localization(path.join(process.cwd(), 'localization'))
